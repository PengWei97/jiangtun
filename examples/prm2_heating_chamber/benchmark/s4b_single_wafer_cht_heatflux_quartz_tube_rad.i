...
[Mesh]
  ...
  # ==================================================
  # 3) Create total interfaces AFTER merge (for CHT)
  # ==================================================

  # total wafer-fluid interface = lr + top + bottom
  [wafer_interface_gen]
    type = SideSetsBetweenSubdomainsGenerator
  ...
  # total tube-fluid interface = top + bottom
  [tube_interface_gen]
    type = SideSetsBetweenSubdomainsGenerator
    input = wafer_interface_gen
    primary_block = 1
    paired_block = 3
    new_boundary = tube_interface
  []

  [./break_sides] # right_to_1, right_to_3
    type = BreakBoundaryOnSubdomainGenerator
    boundaries = 'right left'
    input = tube_interface_gen
  [../]
...
[LinearFVBCs]
  ...
  [walls_u]
    type = LinearFVAdvectionDiffusionFunctorDirichletBC
    variable = vel_x
    boundary = 'wafer_interface tube_interface'
    functor = 0.0
  []
  [walls_v]
    type = LinearFVAdvectionDiffusionFunctorDirichletBC
  ...
  # -----------------------------
  [fluid_wafer_interface_temperature]
    type = LinearFVRobinCHTBC
    variable = T_fluid
    boundary = wafer_interface
    h = ${cht_h_scale}
    incoming_flux = heat_flux_to_fluid_wafer_interface
    surface_temperature = interface_temperature_solid_wafer_interface
    thermal_conductivity = ${alpha_fluid}
  []
  [solid_wafer_interface_flux]
    type = LinearFVRobinCHTBC
    variable = T_solid
    boundary = wafer_interface
    h = ${cht_h_scale}
    incoming_flux = heat_flux_to_solid_wafer_interface
    surface_temperature = interface_temperature_fluid_wafer_interface
  ...
  # fluid-solid CHT interface
  # -----------------------------
  [fluid_tube_interface_temperature]
    type = LinearFVRobinCHTBC
    variable = T_fluid
    boundary = tube_interface
    h = ${cht_h_scale}
    incoming_flux = heat_flux_to_fluid_tube_interface
    surface_temperature = interface_temperature_solid_tube_interface
    thermal_conductivity = ${alpha_fluid}
  []
  [solid_tube_interface_flux]
    type = LinearFVRobinCHTBC
    variable = T_solid
    boundary = tube_interface
    h = ${cht_h_scale}
    incoming_flux = heat_flux_to_solid_tube_interface
    surface_temperature = interface_temperature_fluid_tube_interface
...
[Executioner]
  ...
  cht_interfaces = 'wafer_interface tube_interface'
  cht_solid_flux_relaxation = '0.25 0.25'
  cht_fluid_flux_relaxation = '0.25 0.25'
  cht_solid_temperature_relaxation = '0.40 0.40'
  cht_fluid_temperature_relaxation = '0.40 0.40'
  max_cht_fpi = 5
